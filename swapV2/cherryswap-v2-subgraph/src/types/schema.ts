...
export class AccountTokenHolder extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save AccountTokenHolder entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save AccountTokenHolder entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("AccountTokenHolder", id.toString(), this);
  }

  static load(id: string): AccountTokenHolder | null {
    return store.get("AccountTokenHolder", id) as AccountTokenHolder | null;
  }
  ...
  set height(value: BigInt | null) {
    if (value === null) {
      ...
      this.set("height", Value.fromBigInt(value as BigInt));
    }
  }
