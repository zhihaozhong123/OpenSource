...
interface ReturnMetrics {
  ...
  cherryswapReturn: number // netReturn - hodlReturn
  impLoss: number
  fees: number
}
...
function formatPricesForEarlyTimestamps(position): Position {
  if (position.timestamp < PRICE_DISCOVERY_START_TIMESTAMP) {
    ...
    // WETH price
    if (position.pair?.token0.id === '0x2219845942d28716c0F7C605765fABDcA1a7d9E0') {
      position.token0PriceUSD = 203
    }
    if (position.pair?.token1.id === '0x2219845942d28716c0F7C605765fABDcA1a7d9E0') {
...
export function getMetricsForPositionWindow(positionT0: Position, positionT1: Position): ReturnMetrics {
  ...
  const cherryswap_return = difference_fees_usd + imp_loss_usd
  ...
  return {
    ...
    cherryswapReturn: cherryswap_return,
